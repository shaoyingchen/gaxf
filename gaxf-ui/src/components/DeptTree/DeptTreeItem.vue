...
.tree-item-wrapper { user-select: none; }

.tree-item-row {
  display: flex; align-items: center; gap: 6px; padding: 8px 10px;
  border-radius: 8px; cursor: pointer; transition: all 0.2s;
  font-size: 12px; color: $police-text-muted;

  &:hover { background: rgba(59, 130, 246, 0.05); color: $police-text; }
  &.selected { background: $police-selected-bg; color: $police-blue; font-weight: 500; .folder-icon { color: $police-blue; } }
}

.expand-icon { width: 14px; height: 14px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
.icon-small { width: 14px; height: 14px; }
.folder-icon { flex-shrink: 0; color: $police-text-muted; }
.node-label { flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; line-height: 1.4; }
