...
public class SysDeptController extends BaseController
{
    ...
    /**
     * 获取部门列表
     * 不需要权限校验，管理员可查看所有部门，普通用户只能查看本部门及下级
     */
    @GetMapping("/list")
    public AjaxResult list(SysDept dept)
    {
        ...
        if (!SecurityUtils.isAdmin())
        {
            ...
            // 设置过滤条件：只查询当前用户部门及其下级（ancestors 包含当前部门ID）
            dept.setDeptId(userDeptId);
        }
        List<SysDept> depts = deptService.selectDeptListPublic(dept);
    ...
    public AjaxResult deptTree(SysDept dept)
    {
        ...
        return success(deptService.selectDeptTreeListPublic(dept));
    }
